...
if backend_name == "CudaKernelOps":
    ...
    BACKEND = minitensor.TensorBackend(CudaKernelOps)

BATCH = 10


def RParam(*shape):
    r = 0.1 * (minitensor.rand(shape, backend=BACKEND) - 0.5)
    return minitensor.Parameter(r)


class Linear(minitensor.Module):
...
class SentenceSentimentTrain:
    ...
    def train(
        ...
        optim = minitensor.Adam(self.model.parameters(), learning_rate)
        losses = []
        train_accuracy = []
        validation_accuracy = []
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                # BEGIN ASSIGN1_4
                # TODO
                # 1. Create x and y using minitensor.tensor function through our CudaKernelOps backend
...
if __name__ == "__main__":
    ...
    (X_train, y_train), (X_val, y_val) = encode_sentiment_data(
        ...
        embeddings.GloveEmbedding(
            "wikipedia_gigaword", d_emb=embedding_dim, show_progress=True
        ),
        train_size,
        validation_size,
    )
