...
class Function:
    ...
    def apply(cls, *vals: Tensor) -> Tensor:
        ...
        back = None
        if need_grad:
            back = minitensor.History(cls, ctx, vals)
        return minitensor.Tensor(c._tensor, back, backend=c.backend)
...
class PowerScalar(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        NOTE: minitensor requires that we two gradients: one for the input tensor and scalar.
...
def rand(
    ...
    tensor = minitensor.Tensor.make(vals, shape, backend=backend)
...
def _tensor(
    ...
    Returns:
        new tensor
    """
    tensor = minitensor.Tensor.make(ls, shape, backend=backend)
    tensor.requires_grad_(requires_grad)
    return tensor
...
def tensor_from_numpy(
    ...
    res = minitensor.Tensor(
        v=minitensor.TensorData(
...
def grad_central_difference(
    ...
    """
    Compute the gradient of a function `f` with respect to the `arg`-th input tensor
    at a specific index `ind` using the central difference method.

    Args:
        f: The function to compute the gradient for.
        *vals: The input tensors to the function.
        arg: The index of the tensor to compute the gradient with respect to.
        epsilon: The small perturbation value for central difference.
        ind: The index in the tensor to compute the gradient at.
