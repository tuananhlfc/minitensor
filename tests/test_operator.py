...
from minitensor import MathTest
import minitensor
from minitensor.operators import (
    add,
    addLists,
    eq,
    id,
...
def test_symmetric(a: float, b: float) -> None:
    """Write a test that ensures that :func:`minitensor.operators.mul` is symmetric, i.e.
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    assert_close(mul(a, add(b, c)), add(a * b, a * c))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    expected_ls = [x + y for x, y in zip(ls1, ls2)]
    sum_ls = addLists(ls1, ls2)
    for x, y in zip(sum_ls, expected_ls):
        assert x == y
...
def test_sum(ls: List[float]) -> None:
    assert_close(sum(ls), minitensor.operators.sum(ls))
