...
import minitensor
from minitensor import Tensor, TensorBackend, TensorData, UserIndex, UserShape
...
def vals(draw: DrawFn, size: int, number: SearchStrategy[float]) -> Tensor:
    ...
    return minitensor.tensor(pts)


@composite
def shapes(draw: DrawFn) -> minitensor.UserShape:
...
def tensor_data(
    ...
    if shape is None:
        shape = draw(shapes())
    size = int(minitensor.prod(shape))
    ...
    td = minitensor.TensorData(data, permute_shape)
...
def tensors(
    ...
    backend = minitensor.SimpleBackend if backend is None else backend
    td = draw(tensor_data(numbers, shape=shape))
    return minitensor.Tensor(td, backend=backend)
...
def shaped_tensors(
    ...
    for i in range(n):
        ...
        values.append(
            minitensor.Tensor(
                minitensor.TensorData(data, td.shape, td.strides), backend=backend
            )
        )
    return values
...
def matmul_tensors(
    ...
    for shape in [l1, l2]:
        size = int(minitensor.prod(shape))
        data = draw(lists(numbers, min_size=size, max_size=size))
        values.append(minitensor.Tensor(minitensor.TensorData(data, shape)))
    return values
