...
import minitensor
from minitensor import MathTestVariable, Tensor, TensorBackend, grad_check
from minitensor.cuda_kernel_ops import CudaKernelOps
...
if numba.cuda.is_available():
    ...
    shared["cuda"] = minitensor.TensorBackend(CudaKernelOps)
...
def test_create(backend: str, t1: List[float]) -> None:
    ...
    t2 = minitensor.tensor(t1, backend=shared[backend])
...
def test_cuda_reduce_sum_practice1(
    ...
    b = minitensor.tensor(x)
    s = b.sum()[0]
    b2 = minitensor.tensor(x, backend=shared[backend])
    out = b2.sum(0)
...
def test_cuda_matmul_numpy_eq(m, n, p, backend):
    ...
    c = minitensor.tensor(_a, backend=shared[backend]) @ minitensor.tensor(
...
def test_cuda_matmul_transpose(
    ...
    x = minitensor.tensor(x1, backend=shared[backend])
    y = minitensor.tensor(y1, backend=shared[backend])
...
def test_cuda_permute(backend: str, data: DataObject) -> None:
    ...
    minitensor.grad_check(permute, t1)